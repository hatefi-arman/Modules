    ...
    public class FuelReportDetail
    {
            ...
            ICurrencyDomainService currencyDomainService)
        {
            ...
            if (relevantFuelReportDetailOfTheDayBefore == null)
            {
                ...
                else
                {
                    ...
                    throw new ObjectNotFound(string.Format("RelevantFuelReportDetailOfTheDayBefore for '{0}'", this.Good.Name));
                }
            }
            else
            {
        ...
        private void validateConsumptionAndROBAvailability(double? consumption, double? rob)
        {
                ...
                FuelReport.FuelReportType != FuelReportTypes.EndOfYear)
            {
                if (!(consumption.HasValue && rob.HasValue))
                    throw new BusinessRuleException("BR_FR3", string.Format("Consumption or ROB for '{0}' is Empty.", this.Good.Name));
            }
        }
        ...
        private void validateNotNegativeConsumptionAndROB(double? consumption, double? rob)
        {
                ...
                FuelReport.FuelReportType != FuelReportTypes.EndOfYear)
            {
                if (!(consumption.HasValue && consumption >= 0 &&
                      rob.HasValue && rob >= 0))
                    throw new BusinessRuleException("BR_FR4", string.Format("Consumption or ROB for '{0}'is Negative.", this.Good.Name));
            }
        }
        ...
        private void validateReceiveType(double? receive, ReceiveTypes? receiveType)
        {
                ...
                FuelReport.FuelReportType != FuelReportTypes.EndOfYear)
            {
                ...
                if (receive.HasValue &&
                    ...
                    throw new BusinessRuleException("BR_FR5", string.Format("The ReceiveType for '{0}' is not specified.", this.Good.Name));
            }
        }
        ...
        private void validateTransferType(double? transfer, TransferTypes? transferType)
        {
                ...
                FuelReport.FuelReportType != FuelReportTypes.EndOfYear)
            {
                ...
                if (transfer.HasValue &&
                    ...
                    throw new BusinessRuleException("BR_FR6", string.Format("The TransferType for '{0}' is not specified.", this.Good.Name));
            }
        }
        ...
        private void validateNotNegativeReceiveTransferCorrection(double? receive, double? transfer, double? correction)
        {
                ...
                FuelReport.FuelReportType != FuelReportTypes.EndOfYear)
            {
                if ((receive.HasValue && receive < 0) ||
                    ...
                    throw new BusinessRuleException("BR_FR8", string.Format("Receive or Transfer or Correction for '{0}' is Negative.", this.Good.Name));
            }
        }
            ...
            )
        {
            ...
            if (robOfCurrentDay.HasValue &&
                ...
                throw new BusinessRuleException("BR_FR10", string.Format("ROB for '{0}' is invalid.", this.Good.Name));
        }
                ...
                FuelReport.FuelReportType == FuelReportTypes.EndOfYear)
            {
                    ...
                    correctionPriceCurrencyId.HasValue)
                {
                    throw new BusinessRuleException("BR_FR34",
                                                    string.Format("BR_FR34 : Values assignment for '{0}' is invalid against EndOfVoyage, EndOfMonth or EndOfYear Fuel Report.", this.Good.Name));
                }
            }
        }
        ...
        private void validateRejectTransferReferenceValue(IInventoryManagementDomainService inventoryManagementDomainService)
        {
                ...
                !this.TransferReference.IsEmpty())
            {
                ...
                if (transferRefCount != 1)
                    throw new BusinessRuleException("", string.Format("Reject Transfer Reference Value for '{0}' is invalid.", this.Good.Name));
        ...
        private void validateNotEmptyRejectTransferValues()
        {
            if (this.Transfer.HasValue &&
               this.TransferType.Value == TransferTypes.Rejected &&
               this.TransferReference.IsEmpty())
                throw new BusinessRuleException("", string.Format("Reject Transfer Reference Value for '{0}' is not specified.", this.Good.Name));
        }
        ...
        private void validateInternalTransferReferenceValue(IOrderDomainService orderDomainService)
        {
                ...
                !this.TransferReference.IsEmpty())
            {
                ...
                if (transferRefCount != 1)
                    throw new BusinessRuleException("BR_FR29_2", string.Format("BR_FR19 : Internal Transfer Reference Value for '{0}' is invalid.", this.Good.Name));
                //throw new BusinessRuleException("BR_FR29_2", "BR_FR19 - چنانچه فیلد انتقال مقداردهی گردیده است و نوع آن فروش انتقالی می باشد مرجع آن سفارش  انتقال تائید نهایی شده می باشد");
            }
        }
        ...
        private void validateNotEmptyInternalTransferValues()
        {
            if (this.Transfer.HasValue &&
               this.TransferType.Value == TransferTypes.InternalTransfer &&
               this.TransferReference.IsEmpty())
                throw new BusinessRuleException("BR_FR29_1", string.Format("BR_FR29 : InternalTransfer Reference Value for '{0}' is not specified.", this.Good.Name));
        ...
        private void validateTransferSaleReferenceValue(IOrderDomainService orderDomainService)
        {
                ...
                !this.TransferReference.IsEmpty())
            {
                ...
                if (transferRefCount != 1)
                    throw new BusinessRuleException("BR_FR19", string.Format("BR_FR19: TransferSale Reference Value for '{0}' is invalid.", this.Good.Name));
        ...
        private void validateNotEmptyTransferSaleValues()
        {
            if (this.Transfer.HasValue &&
                this.TransferType.Value == TransferTypes.TransferSale &&
                this.TransferReference.IsEmpty())
                throw new BusinessRuleException("BR_FR 20", string.Format("BR_FR 20 : TransferSale Reference for '{0}' is not specified.", this.Good.Name));
        ...
        private void validateNotEmptyTransferValues()
        {
            if (this.Transfer.HasValue &&
                ...
                throw new BusinessRuleException("", string.Format("TransferType for '{0}' is not specified.", this.Good.Name));
        ...
        private void validateEmptyTransferValues()
        {
            if (!this.Transfer.HasValue &&
                ...
                throw new BusinessRuleException("BR_FR 11", string.Format("BR_FR 11: Transfer for '{0}' is empty but related Values are specified.", this.Good.Name));
        ...
        private void validateTrustReceiveReference(IOrderDomainService orderDomainService)
        {
                ...
                !this.ReceiveReference.IsEmpty())
            {
                ...
                if (receiveRefCount != 1)
                    throw new BusinessRuleException("", string.Format("BR_FR31 : Trust Receive Reference for '{0}' is invalid.", this.Good.Name));
            }
        }
        ...
        private void validateInternalTransferReceiveReference(IOrderDomainService orderDomainService)
        {
                ...
                this.ReceiveType.Value == ReceiveTypes.InternalTransfer)
            {
                ...
                if (receiveRefCount != 1)
                    throw new BusinessRuleException("BR_FR 31", string.Format("BR_FR31 : Internal-Transfer Receive Reference for '{0}' is invalid.", this.Good.Name));
        ...
        private void validateTransferPurchaseReceiveReference(IOrderDomainService orderDomainService)
        {
                ...
                this.ReceiveType.Value == ReceiveTypes.TransferPurchase)
            {
                ...
                if (receiveRefCount != 1)
                    throw new BusinessRuleException("BR_FR 16_2", string.Format("BR_FR 16 : Transfer-Purchase Receive Reference for '{0}' is invalid.", this.Good.Name));
        ...
        private void validatePurchaseReceiveTypeReference(IOrderDomainService orderDomainService)
        {
                ...
                this.ReceiveType.Value == ReceiveTypes.Purchase)
            {
                ...
                if (receiveRefCount != 1)
                    throw new BusinessRuleException("BR_FR 16_1", string.Format("BR_FR 16 : Purchase Receive Type Reference for '{0}' is invlaid.", this.Good.Name));
        ...
        private void validateNotEmptyReceiveReferenceValue()
        {
            if (this.Receive.HasValue)
            {
                if (this.ReceiveType.Value != ReceiveTypes.Trust && this.ReceiveReference.IsEmpty())
                    throw new BusinessRuleException("BR_FR 14", string.Format("BR_FR 14 : Receive Reference for '{0}' is not specified.", this.Good.Name));
        ...
        private void validateNotEmptyReceiveTypeValue()
        {
            if (this.Receive.HasValue &&
                ...
                throw new BusinessRuleException("", string.Format("Receive Type for '{0}' is not specified.", this.Good.Name));
        ...
        private void validateEmptyReceiveValues()
        {
            if (!this.Receive.HasValue &&
                ...
                throw new BusinessRuleException("BR_FR 26", string.Format("BR_FR 26 : Receive for '{0}' is empty but related Values are specified.", this.Good.Name));
            ...
            )
        {
            ...
            validatePositiveCorrectionEmptyReference();

            validateNegativeCorrectionReferenceValue(isTheFirstFuelReport, voyageDomainService);

            validateNotEmptyCorrectionValues();

            validateCorrectionReferenceToBeTheLastIssuedVoyage(isTheFirstFuelReport, fuelReportDomainService);
        }
        ...
        private void validateNegativeCorrectionReferenceValue(bool isTheFirstFuelReport, IVoyageDomainService voyageDomainService)
        {
                ...
                this.CorrectionType.Value == CorrectionTypes.Minus)
            {
                if (!isTheFirstFuelReport &&
                    ...
                    throw new BusinessRuleException("BR_FR 24", string.Format("BR_FR 24 : Reference for Negative Correction of '{0}' is not specified.", this.Good.Name));
        ...
        private void validatePositiveCorrectionEmptyReference()
        {
            if (this.Correction.HasValue &&
                this.CorrectionType.HasValue &&
                ...
                throw new BusinessRuleException("", string.Format("Reference for Positive Correction of '{0}' is specified.", this.Good.Name));
        ...
        private void validateNotEmptyCorrectionValues()
        {
            if (this.Correction.HasValue &&
                ...
                throw new BusinessRuleException("", string.Format("Correction Values for '{0}' are not specified.", this.Good.Name));
            //throw new BusinessRuleException("", "یکی از مقادیر نوع مقدار اصلاحی، مبلغ و یا نوع ارز برای مقدار اصلاحی وارد نشده است.");
        }

        //===================================================================================

        private void validateCorrectionReferenceToBeTheLastIssuedVoyage(bool isTheFirstFuelReport, IFuelReportDomainService fuelReportDomainService)
        {
            if (this.Correction.HasValue &&
                ...
                !this.CorrectionReference.IsEmpty())
            {
                ...
                if (lastIssuedVoyage == null || this.CorrectionReference.ReferenceType != ReferenceType.Voyage ||
                    ...
                    throw new BusinessRuleException("", string.Format("Correction Reference Is not The Last Issued Voyage for '{0}'.", this.Good.Name));
        ...
        private void validateEmptyCorrectionValues()
        {
            if (!this.Correction.HasValue &&
                ...
                throw new BusinessRuleException("BR_FR 25", string.Format("BR_FR 25 : The Correction Values for {0} are not specified correctly.", this.Good.Name));
